...
### Adding a workflow
On your `shr-private` repository, use the GitHub interface to add the following new file `.github/workflows/bx.yml`. This workflow contains a single job called "iarbuild" with 3 steps to build the projects in the sequence `library`, `componentA`, `componentB`, after the repository is checked out:
```yaml
